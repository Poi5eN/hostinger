...
function Create_PrimaryClass() {
  ...
  useEffect(() => {
    ...
    axios.get('http://localhost:4000/api/v1/adminRoute/getAllClass', {withCredentials: true})
      .then((response) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      setLoading(true)
      const response = axios.post('http://localhost:4000/api/v1/adminRoute/createClass', formData, {
  ...
  const handleDelete = async (className) => {
    try {
      const re = await axios.get(`http://localhost:4000/api/v1/adminRoute/getAllClass?className=${className}`, {withCredentials: true});

      // Make an API request to delete the row from the server
      const response = await axios.delete(`http://localhost:4000/api/v1/adminRoute/deleteClass?_id=${re.data.classList[0]._id}`, { withCredentials: true });
  ...
  //   const response = await axios.get(`http://localhost:4000/api/v1/adminRoute/getAllClass?className=${className}`, {withCredentials: true});
  //   console.log("P2 response", response.data.classList);
  //   axios
  //     .delete(`http://localhost:4000/api/v1/adminRoute/deleteClass?_id=${response.data.classList[0]._id}`, { withCredentials: true })
  //     .then((response) => {
  ...
  return (
    <div className=" mt-12  mx-auto p-3">
      <h1 className="text-2xl font-bold mb-4">Create Secondary Class</h1>
      <button
        onClick={openModal}
